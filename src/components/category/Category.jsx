...
/* this card comes from App.jsx and Donated.jsx. initial of the website means '/' card comes from App.jsx but when I clicked 'Donation' in the Navbar, this time card prop comes from Donated.jsx 
...
const Category = ({ card, donationPage }) => {
  /* here color is a object */
  const { id, category, title, img, price, color } = card;
    ...
    <Link to={`/category/${id}`}>
      <div
        ...
        className={`rounded-md ${
          donationPage && "md:flex items-center pb-4 md:pb-0"
        } `}
      >
        <img src={img} alt="" />
        ...
        <div>
          ...
          <p
            ...
            className="rounded-[4px] mt-3 ml-3 inline-block px-2 py-[2px] font-medium"
          >
            {category}
          </p>
          {/* title */}
          <h4
            style={{ color: donationPage ? "#0B0B0B" : textColor }}
            className={`${
              donationPage ? "pb-0" : "pb-4"
          ...
          <p
            ...
            className={`${
              donationPage ? "flex" : "hidden"
            } ml-3 font-semibold mb-4`}
          >
            ${price}
          </p>
          ...
          <button
            ...
            className={`${
              donationPage ? "flex" : "hidden"
            } text-white ml-3 px-4 py-2 rounded-md`}
          >
            View Details
          </button>
        </div>
