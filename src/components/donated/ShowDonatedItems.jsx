...
const ShowDonatedItems = () => {
  ...
  return donated.length > 0 ? (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 justify-center gap-6 pt-3 md:pt-12 px-3 md:px-16 lg:px-28 mb-5 md:mb-10">
        {/* Initially, seeFull is set to false, so the displayed data comes from the filterDonated array (which holds 1 to 4 items). When we click the 'See All' button, seeFull is set to true, and the complete donated array is mapped, allowing us to see 4 above items. */}
        {(seeFull ? donated : filterDonated).map((card, idx) => (
          <Category card={card} donationPage={true} key={idx} />
        ))}
      </div>
      {/* this div use for positioning of button */}
